...
public final class RedisConnection implements RedisApi, Connection {
	...
	@Override
	public Promise<byte @Nullable []> getAsBinary(String key) {
	...
	@Override
	public Promise<byte @Nullable []> getset(String key, byte[] value) {
	...
	@Override
	public Promise<List<byte @Nullable []>> mgetAsBinary(String key, String... otherKeys) {
	...
	@Override
	public Promise<Void> mset(Map<@NotNull String, byte @NotNull []> entries) {
	...
	@Override
	public Promise<Long> msetnx(Map<@NotNull String, byte @NotNull []> entries) {
		if (CHECK) checkArgument(!entries.isEmpty(), "No entry to set");
	...
	@Override
	public Promise<Void> hmset(String key, Map<@NotNull String, byte @NotNull []> entries) {
		if (CHECK) checkArgument(!entries.isEmpty(), "No entry to set");
	...
	private static Promise<byte @Nullable []> parseBulk(RedisResponse response) {
		if (response.isBytes()) {
	...
	private double parseDouble(byte[] bytes) throws RedisException {
		try {
			...
			throw new RedisException(RedisConnection.class, "Could not parse 'double' value: " + new String(bytes));
		}
	}
